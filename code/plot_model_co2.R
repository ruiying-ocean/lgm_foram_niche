...
hist_mean_1960_1990 <-  hist_temp %>% filter(Year > 1960 & Year < 1990) %>% summarise(mean_1960_1990 = mean(Global_Air_Temp_Anomaly)) %>% pull(mean_1960_1990)
hist_mean_1850_1900 <-  hist_temp %>% filter(Year > 1850 & Year < 1900) %>% summarise(mean_1850_1900 = mean(Global_Air_Temp_Anomaly)) %>% pull(mean_1850_1900)
...
model_pco2_temp <- model_pco2_temp %>% group_by(Group) %>%
   mutate(delta_T= Temperature - genie_mean_1850_1900) %>% ungroup()
...
cmip_ts_avg <- cmip_ts %>%
   ...
   summarise(Mean = mean(Monthly_Surf_Temp, na.rm = TRUE),
...
pre_industrial_avg <- cmip_ts_avg %>%
   ...
   summarise(PreIndustrialAvg = mean(Mean, na.rm = TRUE)) %>% pull(PreIndustrialAvg)
...
p4 <- ggplot(cmip_ts_avg, aes(x = Date, y = anomaly, group=experiment)) +
   ...
   labs(x = "", y = "", color = "", fill="", title=
           "Global surface temperature anomaly (°C)") + guides(color = "none")
