...
text_data <- data.frame(
  age = rep(c("modern","future","LGM"),4),
  foram = rep(c("bn", "bs", "sn", "ss"), each=3),
  label = rep(letters[1:4], each=3)
)

p1 <- p1 + geom_text(data=text_data, aes(label=label, group=age), inherit.aes = FALSE,
                x = Inf, y = -Inf, hjust=2, vjust=-1, fontface = "bold", size = 6)
p2 <- p2 + geom_text(data=text_data, aes(label=label, group=age), inherit.aes = FALSE,
                      x = Inf, y = -Inf, hjust=2, vjust=-1, fontface = "bold", size=6)
