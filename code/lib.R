...
smooth_qrg <- function(data, x, y, quant_level = seq(0.9, 0.99, 0.01)) {
    ...
    chart <- chart %>% select(-starts_with("model_y")) %>%
...
convert_to_abundance <- function(data){
    ## carbon quota source
    qc <- data.frame(species = c("bn_c", "bs_c", "sn_c", "ss_c"),
                     # volume in um3
...
plot_tpc <- function(raw_data, smooth_data, x, y){
    ...
    ## plot raw data (dots)
    fig <- ggplot()+ geom_point(data=raw_data, aes(x=!!sym(x), y=!!sym(y), color=age, shape=age), size=0.3, alpha=0.4)
    ...
    fig <- fig + geom_label_repel(data = thermal_opt(smooth_data),
