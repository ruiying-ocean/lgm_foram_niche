...
##### Fig 1a (model)
fig1a <- plot_tpc(raw_data = genie_fg_raw %>% filter(age == "lgm" | age == "pi"), smooth_data = genie_fg_smooth %>% filter(age == "lgm" | age == "pi"), x = "sst", y = "abundance_michaels", vline = FALSE, colors = color_palette[1:2], labels = c("LGM", "PI"))
...
fig1b <- plot_tpc(
  ...
  smooth_data = obs_fg_a_smooth, x = "sst", y = "abundance", vline = FALSE,
...
fake_df <- data.frame(abundance = 0, sst = 0, age = genie_fg_smooth$age, species = genie_fg_smooth$species) %>%
  distinct() %>%
